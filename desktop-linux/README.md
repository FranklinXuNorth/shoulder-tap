...
| 参数 | 含义 |
| --- | --- |
| `--mode` | `tap`（taptap，提醒）、`complete`（拍拍，做完了）、`snap`（响指，模型要问你话）、`bind`（只是拉起常驻）、不认识的一律忽略 |
| `--text` | 这次拍肩为了什么。**不上屏**。`tap` / `snap` 没有它就不拍；`complete` 不需要它 |
...
- 两条道，可以同时在屏上：`tap` / `snap` 挂在屏幕高度 30% 处，`complete` 挂在 52% 处。每条道一个队列，一次播一个。
