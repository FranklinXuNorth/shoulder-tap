...
export function protocolText(): string {
  return [
    ...
    "  拿「用户现在要做的这件事」去比今天清单上所有没做完的任务，只出三档之一：",
    "",
    "  related（有关）—— 就是清单上没做完的某一条（不一定是当前这条），或者是完成它必需的一步。",
    "    → 直接干活。不要提这次检查，不要念清单，不要邀功。",
    "    → 不要自己认定它做完了。只有用户明确说完成，才去 Notion 里把 Status 改成 done。",
    "",
    "  partial（部分有关）—— 沾边但不是清单上任何一条本身：某条任务的外围、顺带的事。",
    ...
    "",
    "第四步 · 做完了就拍拍",
    "  每一轮回答的末尾都打出这只手，一个字符都不要改 —— 它是「这轮到此为止」的记号，桌面靠它拍：",
    "",
    PAT_BLOCK,
    "",
    "  手前面那一句话就是这轮的总结，桌面会把它带在手旁边。同一轮还要提醒（第三步那只手），",
    "  两只都打：拍拍在前，taptap 在后，各自一个代码块，提醒那句话放在最后。漏了手，Stop 钩子会把这轮顶回来让你补。",
    "",
