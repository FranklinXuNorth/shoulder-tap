      ...
      async () =>
        ok(
          [
            ...
            `服务器时间：${new Date().toISOString()}（按 UTC+${process.env.TIMEZONE_OFFSET_HOURS ?? 8} 算今天 = ${today()}）`,
          ].join("\n"),
        ),
    );
 ...
 *     不用改代码，也不用重新 push。
 */
const exported = AUTH_ON ? withMcpAuth(handler, verifyToken, { required: true }) : handler;
