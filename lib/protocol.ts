...
export function protocolText(): string {
  return [
    ...
    "    → 这是唯一会打断他的情况。照下面的格式停下来：",
    "",
    "      1. 先原样打出这只手（代码块，一个字符都不要改）：",
    "",
    HAND_BLOCK,
    "",
    "      2. 一句话点破他在偏离，复述当前那条是什么。",
    "      3. 一个问题：这是必须插队的急事，还是该回去做那条？",
    "      4. 然后停住。**这一轮不要做任何实质性的工作**，一步都不要。",
    "",
    "    → 只有这两种情况才放行：他明确说要改优先级（用 set_focus 重排，或 add_focus 插队），",
    "      或者他明确说当前那条已经做完了（去把 Status 改成 done）。",
    "      他说「无视」也算改优先级 —— 照做，别再提第二遍。",
    "      光是重复一遍请求不算。他没回答，你就还在等。",
    "",
