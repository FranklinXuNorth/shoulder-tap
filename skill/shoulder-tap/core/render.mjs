import { current } from "./focus.mjs";
import { doneBanner, offTaskBanner } from "./ascii.mjs";
/** 把今天的清单画出来。当前那条用 ▸ 标出来，模型和人都一眼能看见。 */
export function renderPlan(items, day) {
    if (!items.length)
        return `${day}：还没有记录任何事。`;
...
export function renderHabits(habits) {
    ...
    return [
        "",
        "【顺便提一句】",
        ...habits.map((h) => `  · ${h.name} —— 超了 ${h.overdueMin} 分钟（说好${h.at ? `每天 ${h.at}` : `每 ${h.everyMin} 分钟一次`}；${h.kind === "soft" ? "软习惯，不能跳过" : "硬习惯，可以今天不做"}）`),
...
/** 做完了就打响指。每次 check_focus 都带上，跟习惯那段一样放在末尾。 */
export function renderDone() {
    return [
        "",
        "【每轮结尾都打响指】回答最末尾打出这只手（代码块，一个字符都别改），手前面那句话就是这轮的总结：",
        "",
        doneBanner(),
        "",
        "同一轮还要 taptap，两只都打：响指在前，taptap 在后，各自一个代码块，提醒那句放最后。漏了手，Stop 钩子会把这轮顶回来让你补。",
    ].join("\n");
}
/** check_focus 的返回。真正干活的是这段给模型看的判断规则。 */
export function renderCheck(items, day, activity, habits = []) {
    ...
    return [
        ...
        `用户现在要做的是：「${activity}」`,
        "",
        `【先判断，再动手】这件事是不是今天清单上某条还没做完的事（不只是第 ${cur.order} 条），或者是完成它必需的一步？`,
        "",
        `A. 是 → 直接继续帮他，不要提这次检查，也不要念清单，也不要因为它不是第 ${cur.order} 条就提醒他。`,
        "   几条可以同时推进，顺序只是建议。" +
            "但不要自己认定哪条做完了：只有用户明确说完成了，才对那一条调用 complete_focus。" +
