...
test("completion caption is the first sentence, markdown stripped, hand cut off", () => {
  const hand = "\n\n```\n  ____/  ________/)   .  |\n```\n第 1 条还没动。";
  assert.equal(doneLine("修好并已**重新发布**。原因：略。" + hand), "修好并已重新发布");
  assert.equal(doneLine("改在 [watch.mjs](a/b.mjs) 里，SHOULDER_TAP_KEY 不动"), "改在 watch.mjs 里，SHOULDER_TAP_KEY 不动");
  assert.equal(doneLine(""), "");
  assert.equal(doneLine("一".repeat(80)).length, 60);
