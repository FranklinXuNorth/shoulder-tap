...
**给 coding agent 装一道注意力护栏。** 你告诉 Claude Code / Codex 今天要做什么。之后你让它干别的，它会**在动手之前**先拍你一下肩膀（「今天不是说先把作品集做完吗？」），然后照样简短地帮你做。

它能做到，是因为它就长在 agent 里面：每次提问、每次调用工具、每轮结束都有钩子，拿你现在要的和你说好要做的比一比。提醒类 app 只能到点叫你；这个在你踩进兔子洞的那一刻就拦住你。
...
顺带：每轮做完，屏幕边上的手打个响指，用一句话告诉你它做了什么，所以它干活时你可以走开。习惯（喝水、健身）只在自然停顿时提一句，不打断你。

> **任务和习惯的数据不上云。** 没有服务器：要么是这台机器上的一个 JSON 文件，要么是你自己的 Notion。
...
设置完之后，同一个图标打开的是首页：**重新走一遍设置**、**手的设置**（样式、停留时间），下面是你的记录 —— 最近两周的待办、在盯的习惯、习惯的历史。右上角可以切中英文。
